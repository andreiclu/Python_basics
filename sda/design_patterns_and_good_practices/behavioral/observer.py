...
class UserObserver(BaseObserver):
    ...
    def handle_message(self, message, user_type):
        # userul curent poate sa primeasca mesajul, doar daca nu vine de la un admin
        if user_type != 'ADMIN':
            print(f"{self._user_name} sees message \'{message}\'")
        else:
            print(f"User {self._user_name} cannot see the message because he's not an admin")
